...
_The full original setup, install, and usage documentation follows. Installation steps are unchanged._
...
```bash
# Add marketplace
/plugin marketplace add <your-internal-remote>/ECC
...
- GitHub source repo: `your-org/ECC`
...
>
> If your local Claude setup was wiped or reset, Start with `node scripts/ecc.js list-installed`, then run `node scripts/ecc.js doctor` and `node scripts/ecc.js repair` before reinstalling anything. That usually restores ECC-managed files without rebuilding your setup.
...
Use `/security-scan` in Claude Code to run it, or add it to your CI pipeline.
...
**History:** This has caused repeated fix/revert cycles in this repo ([#29](<your-internal-remote>/ECC/issues/29), [#52](<your-internal-remote>/ECC/issues/52), [#103](<your-internal-remote>/ECC/issues/103)). The behavior changed between Claude Code versions, leading to confusion. We now have a regression test to prevent this from being reintroduced.
...
- **Codex**: First-class support for both macOS app and CLI, with adapter drift guards and SessionStart fallback. See PR [#257](<your-internal-remote>/ECC/pull/257).
